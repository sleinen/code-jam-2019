...
(defun solve-case (caseno in)
  (let ((P (read in))
    ...
    (let ((points (make-array (list 2) :initial-element '()))
	  (+- (make-array (list 2))))
      (dotimes (axis 2)
	...
	(dotimes (k P)
      ...
      (dotimes (axis 2)
	(setf (aref points axis)
	      (sort (aref points axis) #'<))
	(setf (aref points axis)
	      (let ((x 0))
		(mapcar #'(lambda (point)
			    (cons point
      ...
      (format t "Case #~D: ~D ~D~%"
	      (+ caseno 1)
	      (max-index (aref points 0))
	      (max-index (aref points 1)))))))
